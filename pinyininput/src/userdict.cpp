...
static int gettimeofday(struct timeval *tp, void *) {
	...
	tp->tv_sec = 0;// (long)(current_msecs_since_epoch / 1000);
    tp->tv_usec = 0;// (long)((current_msecs_since_epoch % 1000) * 1000);
    return 0;
}
#endif
...
HANDLE g_mutex_ = CreateMutex(nullptr, FALSE, nullptr);//"KEYBOARD_USERDICT"
...
#define pthread_mutex_lock(HANDLE) (WaitForSingleObject(HANDLE, INFINITE))
#define pthread_mutex_trylock(HANDLE) (WaitForSingleObject(HANDLE, INFINITE))
#define pthread_mutex_unlock(HANDLE) (ReleaseMutex(HANDLE))
